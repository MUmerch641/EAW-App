...
namespace EatWork.Mobile.ViewModels
{
    public class TimeEntryViewModel : BaseViewModel
    {
        ...
        public ICommand TransactionTypeCommand { get; set; }
        ...
        private TimeEntryHolder formHelper_;

        public TimeEntryHolder Holder
        {
            get { return formHelper_; }
            set { formHelper_ = value; RaisePropertyChanged(() => Holder); }
        }

        private readonly ITimeEntryRequestDataService timeEntryDataService_;
        private readonly ICommonDataService myRequestCommonDataService_;
        private readonly IDialogService dialogService_;

        public TimeEntryViewModel(ITimeEntryRequestDataService timeEntryDataService,
            ICommonDataService myRequestCommonDataService)
        {
            timeEntryDataService_ = timeEntryDataService;
        ...
        public void Init(INavigation navigation, long recordId, DateTime? selectedDate)
        {
            ...
            TransactionTypeCommand = new Command<string>(TransactionTypeEvent);
            ...
            ViewTransactionHistoryCommand = new Command(async () => await NavigationService.PushModalAsync(new TransactionHistoryPage(TransactionType.TimeLog, Holder.TimeEntryLogModel.TimeEntryLogId)));
        ...
        private async void InitForm(long recordId, DateTime? selectedDate)
        {
            if (!IsBusy)
            {
                try
                {
                    ...
                    Holder = await timeEntryDataService_.InitForm(recordId, selectedDate);
                }
                catch (Exception ex)
                {
        ...
        private async Task SubmitRequest()
        {
            try
            {
                Holder = await timeEntryDataService_.SubmitRequest(Holder);
        ...
        private async Task TakePhoto()
        {
            try
            {
                var file = await myRequestCommonDataService_.TakePhotoAsync("TIMELOG");
                /*var file = await myRequestCommonDataService_.AttachPhoto2(response, "TIMELOG");*/
        ...
        private async Task CancelRequest()
        {
            try
            {
                ...
                Holder = await timeEntryDataService_.WorkflowTransactionRequest(Holder);
        ...
        private void TransactionTypeEvent(string obj)
        {
            Holder.TimeEntryLogModel.Type = obj;
            Holder = Holder;
        }
            ...
            try
            {
                if (!IsBusy)
                {
                    var param = new FileAttachmentParams()
                    {
                        ...
                        TransactionId = Holder.TimeEntryLogModel.TimeEntryLogId
                    };
        ...
        private void ExecutePageAppearingCommand()
        {
            ...
            MessagingCenter.Subscribe<TimeEntryRequestPage>(this, "onback", (sender) =>
            {
                BackItemPage();
            });
        }
        ...
        private void ExecutePageDisappearingCommand()
        {
            ...
            MessagingCenter.Unsubscribe<TimeEntryRequestPage>(this, "onback");
        }

        protected override async void BackItemPage()
        {
            if (Holder.TimeEntryLogModel.TimeEntryLogId == 0)
            {
                if (!string.IsNullOrWhiteSpace(Holder.TimeEntryLogModel.Remark))
                {
