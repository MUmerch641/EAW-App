...
{
    public class DocumentRequestApprovalViewModel : BaseViewModel
    {
        #region commands
        ...
        private DocumentRequestApprovalHolder formHelper_;

        public DocumentRequestApprovalHolder FormHelper
        {
        ...
        public DocumentRequestApprovalViewModel(IDocumentRequestDataService documentRequestDataService)
        {
        ...
        private async void InitForm(MyApprovalListModel param)
        {
            try
            {
                if (!IsBusy)
                {
                    ...
                    FormHelper = await documentRequestDataService_.InitApprovalForm(param.TransactionTypeId, param.TransactionId);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void WorkflowTransaction(object obj)
        {
            try
            {
                if (obj is Models.DataObjects.WorkflowAction item)
                {
                    ...
                    FormHelper = await documentRequestDataService_.WorkflowTransaction(FormHelper);
