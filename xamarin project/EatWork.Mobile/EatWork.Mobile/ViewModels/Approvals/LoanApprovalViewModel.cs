...
namespace EatWork.Mobile.ViewModels
{
    public class LoanApprovalViewModel : BaseViewModel
    {
        ...
        private LoanRequestApprovalHolder formHelper_;

        public LoanRequestApprovalHolder FormHelper
        {
        ...
        public LoanApprovalViewModel(ILoanRequestDataService loanRequestDataService)
        {
        ...
        private async void InitForm(MyApprovalListModel param)
        {
            try
            {
                if (!IsBusy)
                {
                    ...
                    FormHelper = await loanRequestDataService_.InitApprovalForm(param.TransactionTypeId, param.TransactionId);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void WorkflowTransaction(object obj)
        {
            try
            {
                if (obj is Models.DataObjects.WorkflowAction item)
                {
                    ...
                    FormHelper = await loanRequestDataService_.WorkflowTransaction(FormHelper);
