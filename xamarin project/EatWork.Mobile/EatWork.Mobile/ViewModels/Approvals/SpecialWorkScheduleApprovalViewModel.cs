...
{
    public class SpecialWorkScheduleApprovalViewModel : BaseViewModel
    {
        #region commands
        ...
        private SpecialWorkScheduleApprovalHolder formHelper_;

        public SpecialWorkScheduleApprovalHolder FormHelper
        {
        ...
        public SpecialWorkScheduleApprovalViewModel(ISpecialWorkScheduleDataService specialWorkScheduleDataService)
        {
        ...
        private async void InitForm(MyApprovalListModel param)
        {
            try
            {
                if (!IsBusy)
                {
                    ...
                    FormHelper = await specialWorkScheduleDataService_.InitApprovalForm(param.TransactionTypeId, param.TransactionId);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void WorkflowTransaction(object obj)
        {
            try
            {
                if (obj is Models.DataObjects.WorkflowAction item)
                {
                    ...
                    FormHelper = await specialWorkScheduleDataService_.WorkflowTransaction(FormHelper);
        ...
        private async Task ViewFileAttachments()
        {
            try
            {
                if (!IsBusy)
                {
                    ...
                    {
                        ModuleFormId = 0,//ModuleForms.ChangeWorkScheduleRequest,
                        TransactionId = 0//FormHelper.TransactionId
                    };
