...
{
    public class TimeEntryApprovalViewModel : BaseViewModel
    {
        #region commands
        ...
        private TimeEntryApprovalHolder formHelper_;

        public TimeEntryApprovalHolder FormHelper
        {
            get { return formHelper_; }
            set { formHelper_ = value; RaisePropertyChanged(() => FormHelper); }
        }

        private readonly ITimeEntryRequestDataService timeEntryRequestDataService_;

        public TimeEntryApprovalViewModel(ITimeEntryRequestDataService timeEntryRequestDataService)
        {
            timeEntryRequestDataService_ = timeEntryRequestDataService;
        }
        ...
        private async void InitForm(MyApprovalListModel param)
        {
            try
            {
                if (!IsBusy)
                {
                    ...
                    FormHelper = await timeEntryRequestDataService_.InitApprovalForm(param.TransactionTypeId, param.TransactionId);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void WorkflowTransaction(object obj)
        {
            try
            {
                if (obj is Models.DataObjects.WorkflowAction item)
                {
                    ...
                    FormHelper = await timeEntryRequestDataService_.WorkflowTransaction(FormHelper);
