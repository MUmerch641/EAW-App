...
namespace EatWork.Mobile.Services.TestServices
{
    public class WorkflowDataService : IWorkflowDataService
    {
        ...
        public async Task<List<TransactionHistory>> GetTransactionHistory(long TransactionTypeId, long transactionId)
        {
            ...
            var temp = new List<TransactionHistory>();
            ...
            try
            {
                ...
                for (int i = 1; i <= 5; i++)
                {
                    ...
                    {
                        RequestersName = "Hulburt Clemmy",
                        ApproversName = "",
                        TransactionType = "Overtime Request",
                        StageDescription = "Final Approval",
                        NextApprovers = "Hulburt Clemmy",
                        ActionTypeId = 0,
                        ActionPastTense = "",
                        MessageTemplate = "<b>{{TransactionType}}</b> has been submitted to <b>{{NextApprovers}}</b> for <b>{{StageDescription}}</b>",
                        Remarks = "",
                        DetailedMessage = "",
                        HistoryTypeId = 3,
                        LogDate = Convert.ToDateTime("2018-10-10 21:38:23.877"),
                        CreatedBy = ""
                    }); ;
                }
                ...
                foreach (var item in temp)
                {
                    ...
                    PropertyCopier<Models.TransactionHistory, Models.TransactionHistory>.Copy(item, model);
                    model.MessageTemplate = string_.StringBinder<TransactionHistory>(item.MessageTemplate, item, "{{", "}}");
                    retValue.Add(model);
                }
